...
## [Unreleased] - Rust-first CLI migration (in progress)

A standalone, Python-free Rust CLI rewrite is in progress under
`lockknife-cli-rs/`, alongside the existing Python + Rust hybrid architecture
described below. See `lockknife-cli-rs/MIGRATION_SCOPE.md` and
`lockknife-cli-rs/PROGRESS.md` for current scope and increment-by-increment
status. `lockknife-core` and `lockknife-tui` remain in the workspace as
parked crates (workspace members, excluded from `default-members`) holding
the deferred/sidecar scope (exploit/network/pcap/gesture/crypto-wallet/
intel/yara-x, and the PyO3-bound legacy TUI) until those phases begin.
