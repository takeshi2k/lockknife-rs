...
> **Rust-first migration in progress.** A second, in-progress rewrite is
> underway in [`lockknife-cli-rs/`](./lockknife-cli-rs), moving the
> deterministic, practical-to-compile parts of this tool (CLI, config, TUI,
> ADB core, offline credential cracking, extraction, forensics, HTML/JSON/CSV
> reporting, chain-of-custody, security/OWASP checks, APK static analysis)
> into a standalone, dependency-light Rust binary with no PyO3/Python
> dependency at runtime. `Frida`, `PDF` reports, `ML/AI`, `threat-intel`,
> `network/PCAP`, `crypto-wallet`, `YARA` rule scanning, and the best-effort
> device-cracking extras (`gesture`/`WiFi`/`keystore`/`passkeys`) are kept
> deferred or as an external/sidecar concern for this phase — see
> [`lockknife-cli-rs/MIGRATION_SCOPE.md`](./lockknife-cli-rs/MIGRATION_SCOPE.md)
> and [`lockknife-cli-rs/PROGRESS.md`](./lockknife-cli-rs/PROGRESS.md) for the
> exact scope and increment log. The rest of this README describes the
> existing Python + Rust hybrid architecture below, which remains the
> production architecture until the Rust-first crate reaches parity.
